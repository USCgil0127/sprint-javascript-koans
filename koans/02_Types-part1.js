describe('type에 대해서 학습합니다.', function () {
  it("비교연산자 '=='는 두 값의 일치 여부를 느슨하게 검사(loose equality)합니다.", function () {
    ...
    // 이래서 느슨한 연산자는 쓰지 않는 게 좋겠다. 어떤 결과값이 나올지 예상이 안됨.
    /*
  ...
  */
  ...
  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    // 솔직히 이 결과값은 의외다. '2'라고 생각했는데... 근데, 숫자가 아니라, char라고 생각하면 이해가 된다.
    expect(1 + '1').to.equal('11');
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    // 아니 이게 왜 숫자로 변환되고 빼지는 거임??
    // 그냥 알아둬야 겠다... 단, char에서 이런 연산이 일어나면 NaN가 나온다.
    expect(123 - '1').to.equal(122);
  });

  it('expect의 인자로 들어간 표현식의 평가(evaluation) 결과를 예측해 봅니다.', function () {
    // 이게 왜 정답이 2인가?? 0은 거짓이고, 1은 사실 이라서??
